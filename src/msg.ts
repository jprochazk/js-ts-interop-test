export class Message {
  ...
  tag<T extends TagTypes = "raw">(key: keyof KnownTags, type?: T): TagType<T> | null;
  tag<T extends TagTypes = "raw">(key: string, type?: T): TagType<T> | null;
  tag<T extends TagTypes = "raw">(key: keyof KnownTags | string, type?: T): TagType<T> | null {
    const v = this.tags?.[key];
    if (!v) return null;
    if (type === "string") return unescape(v) as any;
    if (type === "csv") return v.split(",") as any;
    if (type === "number") return Number(v) as any;
    return v as any;
  }
...
type TagTypes = "raw" | "string" | "number" | "csv";
// prettier-ignore
type TagType<T extends TagTypes> =
